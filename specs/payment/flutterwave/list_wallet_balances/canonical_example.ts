...
interface ListWalletBalancesInput {
  trace_id?: string;
}
...
interface ListWalletBalancesResponse {
  status: string;
  message?: string;
  data: WalletBalance[];
}
...
export async function listWalletBalances(
...
): Promise<ListWalletBalancesResponse> {
  const token = await getAccessToken();

  const url = `${API_BASE_URL}/wallets/balances`;
  ...
  return response.json() as Promise<ListWalletBalancesResponse>;
}

/*
Usage example:

const result = await listWalletBalances();

for (const wallet of result.data) {
  console.log(wallet.currency, wallet.available_balance);
}

// Use the currency field to drive a treasury dashboard with one row per wallet.
*/
