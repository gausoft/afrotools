...
type CountryCode =
  | "CM" | "CI" | "CG" | "EG" | "ET" | "GA" | "GH" | "IN" | "KE"
  | "MW" | "NG" | "RW" | "SL" | "SN" | "TD" | "TZ" | "UG" | "US" | "ZA" | "ZM";

interface ListBanksInput {
  country: CountryCode;
  trace_id?: string;
}
...
export async function listBanks(input: ListBanksInput): Promise<ListBanksResponse> {
  const token = await getAccessToken();

  const params = new URLSearchParams();
  params.set("country", input.country);

  const url = `${API_BASE_URL}/banks?${params.toString()}`;
  ...
  return response.json() as Promise<ListBanksResponse>;
}

/*
Usage example:

const result = await listBanks({ country: "NG" });

for (const bank of result.data) {
  console.log(bank.id, bank.code, bank.name);
}

// Use bank.id (or bank.code, depending on payout flow) when building a bank-payout payload.
*/
