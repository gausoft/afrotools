...
interface CreateOrchestratorOrderInput {
  amount: number;
  currency: string;
  reference: string;
...
interface OrchestratorOrderData {
  id: string;
  amount: number;
  currency: string;
  reference: string;
...
interface CreateOrchestratorOrderResponse {
  status: "success" | "failed";
  message?: string;
  data: OrchestratorOrderData;
}
...
export async function createOrchestratorOrder(
  input: CreateOrchestratorOrderInput
): Promise<CreateOrchestratorOrderResponse> {
  ...
  const response = await fetch(`${API_BASE_URL}/orchestration/direct-orders`, {
    method: "POST",
    headers,
  ...
  return response.json() as Promise<CreateOrchestratorOrderResponse>;
}

/*
Usage example:

const order = await createOrchestratorOrder({
  amount: 10000,
  currency: "NGN",
  reference: "preauth-2024-000123",
  customer: {
...
// To capture funds against the order, call create_charge with order_id = order.data.id:
//
// await createCharge({
//   amount: 10000,
//   currency: "NGN",
//   reference: "capture-2024-000123",
//   customer_id: order.data.customer_id,
//   payment_method_id: order.data.payment_method_details!.id,
//   order_id: order.data.id,
//   trace_id: "trace-capture-2024-000123",
// });
*/
